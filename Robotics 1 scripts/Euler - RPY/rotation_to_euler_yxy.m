function to_return = rotation_to_euler_yxy(R)
    % rotation_to_euler_yxy: Calcola gli angoli di Euler da una matrice di rotazione R.
    % La combinazione di Euler in questo caso è YXY.
    %
    % Inputs:
    ...
    if s2 ~= 0
        ...
        disp(['c3 = -r23/s2 = ', num2str(c3)]);
        disp(['s3 = r21/s2 = ', num2str(s3)]);
        disp(['c1 = r32/s2 = ', num2str(c1)]);
        disp(['s1 = r12/s2 = ', num2str(s1)]);
    ...
    elseif s2 == 0 && c2 > 0
        disp('Caso speciale: angolo DUE = 0');
    ...
    elseif s2 == 0 && c2 < 0
        disp('Caso speciale: angolo DUE = +/- pi');
        disp("--------conosci solo sen/cos della somma di angoli--------");        
        disp(['cos(uno - tre) = r11 = ', num2str(R(1,1))]);
        disp(['-sin(uno - tre) = r13 = ', num2str(R(1,3))]);
        uno = NaN; % Indica che il valore non è calcolabile
